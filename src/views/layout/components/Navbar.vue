<template>
  <div class="nav-bar" style="padding-left: 32px;">
    <el-menu class="navbar" mode="horizontal">
      ...
      <span v-if="isBindBIM===true" class="bim-name-area">{{bim_name_desc}}</span>
      <div class="right-menu">
        <el-button type="primary" style="top:0px;position: relative;" @click="openercode">
          <img
            src="../../../../static/icon/icon.png"
            alt
            style="width: 20px;height: 20px;color: #eaeefb"
          />
          <span>系统二维码生成</span>
        </el-button>
        <el-button type="primary" style="top:0px;position: relative;" @click="openScreenHandle">
          <icon name="desktop" scale="1.4" style="line-height: 0px;"></icon>
          <span>项目看板</span>
        </el-button>
        <el-button
          v-show="this.project_id!==null"
          type="primary"
          style="top:0px;position: relative;"
          @click="openDoorScreenHandle"
        >
          <icon name="tv" scale="1.4" style="line-height: 0px;"></icon>
          <span>现场大屏</span>
        </el-button>
...
export default {
  ...
  methods: {
    ...
    openDoorScreenHandle() {
      window.open(
        `http://w.yidebim.com:8899/entrance/#/main/index?project_id=${
          this.project_id
        }&type=1&tk=${getToken()}`,
        '_blank'
      )
