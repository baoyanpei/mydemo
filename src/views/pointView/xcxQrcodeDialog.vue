...
<template>
  <div id="xcx-qrcode-dialog" class="xcx-qrcode-dialog">
    <el-dialog
      ...
      top="10vh"
      :lock-scroll="true"
      :visible.sync="ViewPointQrcodeDialog.show"
...
export default {
  ...
  methods: {
    ...
    async openedDialogHandle() {
      ...
      this.img_src_logo = `${process.env.BASE_DOMAIN_ORG_URL}${this.currentProject.org_id}/02.png`
      let _title = ''
      ...
      setTimeout(() => {
        let _qrcode_url = `https://xcx.tddata.net/smz/#/xcx/pvshow?project_id=${_data.project_id}&pvid=${_data.id}&from=xcx_pv_qrcode`
