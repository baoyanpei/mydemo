<style lang="scss">
  @import "./tajiListDialog";

</style>
<template>

  <div id="taji-list-dialog" class="taji-list-dialog">
    <el-dialog :modal="false" width="450px" top="10vh" left="100" :lock-scroll="true" :close-on-click-modal="false"
      :close-on-press-escape="true" :visible.sync="TajiListDialog.show" @opened="openedDialogHandle"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      TajiListDialog: {
        get: function () {
          return this.$store.state.loT.TajiListDialog
        },
        set: function (newValue) {
          this.$store.state.loT.TajiListDialog = newValue
        }
      },
    ...
    methods: {
      ...
      async openedDialogHandle() {
        ...
        this.buildItem = this.TajiListDialog.buildItem
        console.log('this.TajiListDialog', this.TajiListDialog)
        this.tipMessage = "正在查询..."
        this.deviceList = await this.getDeviceConfigList()
        if (this.deviceList.length === 0) {
          this.tipMessage = "当前建筑中没有配置塔吊模型"
          return
        }
        this.tipMessage = ""
      },
