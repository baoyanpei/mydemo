...
<template>
  <div id="model-display-index" class="model-display-index" style="margin: 0px;">
    ...
    <div v-if="isShowViewPointArea" class="viewPointShowArea">
      <div class="viewPointTitle">
        <div class="title">
          ...
          <span v-show="viewPointTitleName!==''">> {{viewPointTitleName}}</span>
        </div>
        <el-button
          type="danger"
          class="btn-close-view-point"
          @click="exitRestoreHandle"
          size="small"
          style="width:130px;"
        >
    ...
    <div
      v-show="isShowViewPointArea === true && ViewPointType===1 && viewPointImgTopUrl !==''"
    ...
    <div
      v-show="isShowViewPointArea === true && ViewPointType===1 && viewPointImgSideUrl !==''"
    ...
    <div v-if="isShowToolbarMarker" class="toolbar-marker">
      ...
      <el-button class="marker-button" title="保存当前视点">
        <font-awesome-icon
          v-if="isSaveViewValid === true"
          :icon="['far','save']"
          @click="showSaveViewsPointHandle(0)"
        />
        <font-awesome-icon
          v-if="isSaveViewValid === false"
          style="color:grey;"
          :icon="['far','save']"
        />
      </el-button>
    ...
    <markupTools
      ref="MarkupTools"
      v-show="isShowToolbarMarkerStyle"
      v-on:setMarkerMode="setMarkerMode"
...
export default {
  ...
  methods: {
    ...
    addCustomToolBar() {
      ...
      // 恢复标注功能
      let buttonRestoreMarker = new Autodesk.Viewing.UI.Button(
        'my-restore-marker-button'
      )
      buttonRestoreMarker.icon.style.backgroundImage =
        'url(./static/icon/ico_restoreMarkup.png)'
    ...
    addViewpointToolBar() {
      ...
      buttonMarker.icon.style.backgroundImage =
      ...
      if (this.itemList.length === 1) {
        buttonMarker2.icon.style.backgroundImage =
        ...
        buttonMarker2.icon.style.backgroundImage =
          'url(./static/icon/ico_markup_grey.png)'
        buttonMarker2.setToolTip('合并模型不能进行位置标注')
      }

      // 视点管理功能
      let buttonViewPoint = new Autodesk.Viewing.UI.Button(
        'my-marker-manager-button'
      )
      buttonViewPoint.icon.style.backgroundImage =
        'url(./static/icon/ico_layer3.png)'
      ...
      // SubToolbar
      this.ControlGroupViewPoint = new Autodesk.Viewing.UI.ControlGroup(
    ...
    showAllViewpointToolBar() {
      ...
      // 查看所有标注点
      let buttonMarker = new Autodesk.Viewing.UI.Button(
        'show-view-point-button'
      )
      buttonMarker.icon.style.backgroundImage =
        'url(./static/icon/ico_show_all_view_points.png)'
      buttonMarker.onClick = e => {
      ...
      let buttonTaskMarker = new Autodesk.Viewing.UI.Button(
        'show-view-point-task-button'
      )
      buttonTaskMarker.icon.style.backgroundImage =
        'url(./static/icon/ico_viewpoint_task.png)'
      ...
      // 关闭所有标注点
      let buttonCloseAllViewPoint = new Autodesk.Viewing.UI.Button(
        'my-marker-manager-button'
      )
      buttonCloseAllViewPoint.icon.style.backgroundImage =
        'url(./static/icon/ico_show_all_view_points_x1.png)'
      ...
      this.ControlGroupShowAllViewPoint = new Autodesk.Viewing.UI.ControlGroup(
        'show-view-point-toolbar'
      )
    ...
    onSelectionChanged1(event) {
      ...
      // Asyncronous method that gets object properties
      // 异步获取模型的属性
      this.viewer.getProperties(_dbIds[0], elements => {
        var dbid = elements.dbId
        console.log('elements', elements)
        // let min = this.getFragXYZ(dbid)
        // this.drawViewPointLabel(min, 'aaa', 'asd', 'dasd')
        ...
        case 2: // 2-普通视点
          this.viewer
            ...
            .then(markupsExt => {
              ...
              this.markupsExt = this.viewer.getExtension(
                'Autodesk.Viewing.MarkupsCore'
              )
              ...
              this.styleObject = this.nsu.createStyleSheet(
                this.styleAttributes,
                this.markupsExt.viewer
              )
    ...
    setMarkerMode(type) {
      ...
      switch (type) {
        case 'Rectangle':
          _EditMode = new Autodesk.Viewing.Extensions.Markups.Core.EditModeRectangle(
            this.markupsExt
          )
          break
        case 'Circle':
          _EditMode = new Autodesk.Viewing.Extensions.Markups.Core.EditModeCircle(
            this.markupsExt
          )
          break
        case 'Arrow':
          _EditMode = new Autodesk.Viewing.Extensions.Markups.Core.EditModeArrow(
            this.markupsExt
          )
          break
        case 'Freehand':
          _EditMode = new Autodesk.Viewing.Extensions.Markups.Core.EditModeFreehand(
            this.markupsExt
          )
          break
        case 'Cloud':
          _EditMode = new Autodesk.Viewing.Extensions.Markups.Core.EditModeCloud(
            this.markupsExt
          )
          break
        case 'Text':
          _EditMode = new Autodesk.Viewing.Extensions.Markups.Core.EditModeText(
            this.markupsExt
          )
          break
        case 'Polyline':
          _EditMode = new Autodesk.Viewing.Extensions.Markups.Core.EditModePolyline(
            this.markupsExt
          )
          break
        case 'Polycloud':
          _EditMode = new Autodesk.Viewing.Extensions.Markups.Core.EditModePolycloud(
            this.markupsExt
          )
          break
        case 'Highlight':
          _EditMode = new Autodesk.Viewing.Extensions.Markups.Core.EditModeHighlight(
            this.markupsExt
          )
          break
      }
    ...
    subscribeToAllEvents() {
      for (var key in Autodesk.Viewing) {
        if (key.endsWith('_EVENT')) {
          ;(eventName => {
            this.viewer.addEventListener(Autodesk.Viewing[eventName], event => {
          ...
          .then(markupsExt => {
            ...
            setTimeout(() => {
              ...
              if (_pointType === 1) {
                //位置视点
                this.$store
                  .dispatch('ShowPositionViewPointSaveDialog', param)
                  .then(() => {})
                  .catch(() => {})
              } else {
                this.$store
                  .dispatch('ShowViewPointSaveDialog', param)
                  .then(() => {})
                  .catch(() => {})
              }
            }, 1000)
    ...
    openViewPointManageHandle() {
      ...
      // this.$store.dispatch('SetVideoDialog', param).then(() => {}).catch(() => {})
      this.$store
        .dispatch('ShowViewPointManagerDialog', param)
        .then(() => {})
        .catch(() => {})
    },
    // draw1() {
    ...
    async ShowViewPoint() {
      ...
      switch (_pointType) {
        case 1: // 1-基于项目的公共位置视点
          ...
          // this.isShowSaveMarkerArea = false
          console.log(
            'this.ViewPointCurrentData.camera_info',
            this.ViewPointCurrentData.camera_info
          )
          let camera_info = JSON.parse(
        ...
        case 2: // 2-普通视点
          ...
          console.log(
            'itemCurrentItemIdList123123',
            this.itemCurrentItemIdList,
            item_ids_list
          )
          if (
            this.itemCurrentItemIdList.sort().toString() !==
            item_ids_list.sort().toString()
          ) {
          ...
          this.viewer
            ...
            .then(markupsExt => {
              ...
              let _marekup_svg = Base64.decode(
                this.ViewPointCurrentData.svg_info
              )
              let camera_info = JSON.parse(
                Base64.decode(this.ViewPointCurrentData.camera_info)
              )
              ...
              setTimeout(() => {
                ...
                markupsExt.show()
                markupsExt.loadMarkups(
                  _marekup_svg,
                  'markup' + this.ViewPointCurrentData.id
                )
