...
<template>
  <div class="main-container" style="margin: 0px;">
    ...
    <div class="temperature">
      <h1>环境监测</h1>
      <div class="huanjingbox" v-show="huanjingshow2">暂无环境数据</div>
      <div v-show="huanjingshow">
        <div class="huanjinline" style="width: 100%;height: 30px;padding: 0 75px 0 20px">
          <span style="float: left;line-height: 30px;font-weight: 600;color: #FF0000;">
            温度:
            <span>{{temp}}℃</span>
          </span>
          <span style="float: right;line-height: 30px;font-weight: 600;color: #FF0000;">
            湿度:
            <span>{{shiduh}}%</span>
          </span>
        </div>
    ...
    <div class="newtask">
      ...
      <div class="taskbottom" style="width: 100%;overflow: hidden;border: 1px solid #BABABA">
        <div
          class="tasksmall"
          v-for="item in this.taskbox"
          @click="newtaskdetailsfnc(item)"
        ...
        >
          <div class="taskbanner" style="height: 20px;border-bottom: 1px solid #e7e7e7">
            <img
              src="/static/icon/BrowserPreview_tmp%20(2).png"
              alt
              style="width: 15px;height: 15px;margin-top: 2px;margin-left: 8px;float: left"
            />
            <span
              style="font-size: 13px;display: block;float: left;margin-top: 3px;margin-left: 10px;font-weight: 800"
            >{{item.first}}</span>
            <span
              style="font-size: 13px;display: block;float: left;margin-top: 3px;margin-left: 10px;font-weight: 800"
            >—</span>
            <span
              style="font-size: 13px;display: block;float: left;margin-top: 3px;margin-left: 10px;font-weight: 800"
            >{{item.questions_type}}</span>
            <div
              class="statusbox"
              style="float: right;width: 40px;height: 14px;font-size: 10px;margin-top: 3px;margin-right: -5px;text-align: center;line-height: 14px;color: #eaeefb"
          ...
          <div class="tasknei" style="width: 100%;height: 108px;">
            <div class="taskneileft" style="width: 120px;height: 100%;float: left">
              <img
                :src="item.imgurl"
                alt
                style="width: 110px;height: 100px;margin-top: 5px;margin-left: 5px"
              />
            </div>
            <div class="taskright" style="float: left;width: 262px;height: 100%;position: relative">
              <span
                style="margin-top: 5px;display: block;overflow: hidden;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;"
              >{{item.title}}</span>
              <span style="display: block;font-size: 12px;color: #a8a8a8;margin-top: 5px">
                发起人:
                <span style="color: #000000">{{item.originator}}</span>
              </span>
              <span style="display: block;font-size: 12px;color: #a8a8a8;margin-top: 5px">
                发起时间:
                <span style="color: #000000">{{item.sendTime}}</span>
              </span>
              <div
                class="taskrightbottom"
                style="position: absolute;bottom: 0;height: 25px;width:100%;"
              >
                <div class="logobox">{{item.first}}</div>
                <div class="logobox">{{item.questions_type}}</div>
                <div class="star_block" style="float: right;">
    ...
    <div class="equipment" style="border: 1px solid #BABABA">
      ...
      <div
        class="shuidian"
        style="width: 95%;margin:10px auto;height: 100px;border: 1px solid #e5e5e5;border-radius: 10px;"
      >
        <div
          class="shuidianleft"
          style="width: 50%;height: 80%;border-right: 1px solid #e5e5e5;margin-top: 10px;float: left"
        >
          <img
            src="../../../static/mainpng/3522ca9f171f5bc9cd2400a2534b40b.png"
            alt
            style="width: 60px;height: 60px;margin-top: 10px;margin-left: 20px;float: left"
          />
          <div
            class="dianbiao"
            style="width: 180px;height: 100%;float: left;margin-left: 30px;"
            v-show="dianbiaoshow"
          >
            <span style="font-size: 15px;display: block;margin-top: 10px;color: #909399">
              总读数:
              <span>{{eleallnum}}</span>
            </span>
            <br />
            <span style="font-size: 18px;color: #909399">
              今日用量:
              <span>{{todayelenum}}度</span>
            </span>
          </div>
          <div
            class="dianbiaonone"
            style="float: left;width: 180px;height: 100%;"
            v-show="dianbiaoshow2"
          >暂无电表信息</div>
        </div>
        <div class="shuidianleft" style="width: 50%;height: 80%;margin-top: 10px;float: left">
          <img
            src="../../../static/mainpng/shuibiao.png"
            alt
            style="width: 60px;height: 60px;margin-top: 10px;margin-left: 20px;float: left"
          />
          <div
          ...
          >
            <span style="font-size: 15px;display: block;margin-top: 10px;color: #909399">
              总读数:
              <span>{{waterallnum}}</span>
            </span>
            <br />
            <span style="font-size: 18px;color: #909399">
              今日用量:
              <span>{{todaywaternum}}吨</span>
            </span>
          </div>
          <div
            class="shuibiaonohave"
            style="float: left;width: 180px;height: 100%;"
            v-show="shuibiaoshow2"
          >暂无水表信息</div>
        </div>
      </div>
      <div v-for="item in this.tajidatabox">
        ...
        <div class="tajiright">
          ...
          <div class="tajibox" style="width: 100%;height: 25px;margin-top: 10px;">
            <div
              class="tajismallbox"
              v-show="tajismallboxshow"
              style="width: 60px;border: 1px solid #1abc9c;;height: 100%;text-align: center;line-height: 25px;font-size: 14px;color: #1abc9c;font-weight: 800;"
            >{{item.tajiname}}</div>
          </div>
          <!--塔机角度 转角-->
          <div class="tajidata" v-show="tajishow">
            <div class="tajijiaodu" style="width: 100%;height: 50px;margin-top: 5px;">
              <div class="tajijiaoduleft">
                <img
                  src="../../../static/mainpng/jiaodu.png"
                  alt
                  style="float:left;width: 40px;height: 40px;margin-left: 40px;margin-top: 5px"
                />
            ...
            <div class="tajijiaodu" style="width: 100%;height: 50px;margin-top: 5px;">
              <div class="tajijiaoduleft">
                <img
                  src="../../../static/mainpng/huizhuan.png"
                  alt
                  style="float:left;width: 40px;height: 40px;margin-left: 40px;margin-top: 5px"
                />
            ...
            <div class="tajijiaodu" style="width: 100%;height: 50px;margin-top: 5px;">
              ...
              <div class="tajijiaoduleft">
                <img
                  src="../../../static/mainpng/zhongliang.png"
                  alt
                  style="float:left;width: 40px;height: 40px;margin-left: 40px;margin-top: 5px"
                />
          ...
          <!--没有塔机数据的时候-->
          <div class="nohavetajidata" v-show="tajishow2">暂无塔机数据</div>
        </div>
      </div>
    </div>
    <!--进出车辆-->
    ...
    <el-dialog
      v-el-drag-dialog
      custom-class="HuiyiDialog"
      :modal="false"
      width="400px"
      top="15vh"
      :lock-scroll="true"
      :close-on-click-modal="false"
      v-if="dialogCJHYVisible"
      :visible.sync="dialogCJHYVisible"
      title="创建计划"
    ...
    <el-dialog
      ...
      :lock-scroll="true"
      :close-on-click-modal="false"
      v-if="showHuiyiFullCalendar"
      :visible.sync="showHuiyiFullCalendar"
      title="会议计划"
      @dragDialog="handleDrag"
    >
