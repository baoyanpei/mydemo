...
function initCar(towerGroup, Tname, Tdistance) {
  // 移动小车的位置
  towerGroup
    .getObjectByName(Tname, true)
    .getObjectByName("Car", true).position.x = Tdistance;
}
...
function initHook(towerGroup, Tname, Tdistance, Tlength) {
  ...
  towerGroup
    .getObjectByName(Tname, true)
    .getObjectByName("Hook", true).position.x = Tdistance;
  // 通过线段数量确定吊钩的高度
  towerGroup
    .getObjectByName(Tname, true)
    .getObjectByName("Hook", true).position.z = -(lNum * 0.5);
}
...
function updateTower(isEx, degree, distance, length) {
  ...
  for (var i = 1; i <= maxNum; i++) {
    ...
    isEx.getObjectByName("Line_" + String(i), true).visible = !(i > lNum);
  }
  // 根据线段情况更新吊钩高度
...
function createSectionHigh(towerGroup, Tname, Thigh) {
  // 初始化标准节
  var section = towerGroup
    .getObjectByName(Tname + "_U", true)
    .getObjectByName("Section", true);
  // 计算标准节数量
